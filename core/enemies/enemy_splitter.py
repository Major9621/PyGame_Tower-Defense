...
class SplitterEnemy(Enemy):
    def __init__(self, path, gold_manager, hp_multiplier):
        ...
        self.health = ENEMY_HEALTH * 1.5 * hp_multiplier
    ...
    def die(self):
        if self.state != "die":
            small_enemy = SmallSplitEnemy(self.path[self.path_index:], self.gold_manger, self.hp_multiplier)
...
class SmallSplitEnemy(Enemy):
    def __init__(self, path, gold_manager, hp_multiplier):
        super().__init__(path, gold_manager, hp_multiplier)
        self.health = ENEMY_HEALTH * 0.5 * hp_multiplier
        self.max_health = self.health
